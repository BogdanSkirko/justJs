...
const checkDogs = function (dogsJulia, dogsKate) {
  ...
  const realDogsAll = [...realDogsJulia, ...dogsKate];
  console.log(realDogsAll);
  realDogsAll.map((val, i) =>
    val >= 3
      ? console.log(`Dog number${i + 1} is an adult, and is ${val} years old `)
      : console.log(`Dog number${i + 1} is still a puppy`)
  );

  // for ([i, age] of realDogsAll.entries()) {
  //   age >= 3
...
// 1)
// for (pet of dogs) {
//   pet.recomendedFood = +(pet.weight ** 0.75 * 28).toFixed(2);
// }
dogs.forEach(dog => (dog.recomendedFood = Math.trunc(dog.weight ** 0.75 * 28)));

// 2)
const ownerSarah = dogs.find(owner => owner.owners.includes('Sarah'));
ownerSarah.recomendedFood > ownerSarah.curFood
  ? console.log('Sarahs its eating to little')
  : console.log('Sarahs its eating too much');
...
console.log(`${ownerEatToLittle.join(' and ')}s dogs eat to little`);
...
const checkEaitingNormaly = dog =>
  dog.curFood > dog.recomendedFood * 0.9 &&
  dog.curFood < dog.recomendedFood * 1.1;

console.log(dogs.some(checkEaitingNormaly));

// 7)
console.log(dogs.filter(checkEaitingNormaly));
